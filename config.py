...
class Config(metaclass=ConfigMeta):
    ...
    def _get_config_value(cls, name: str) -> Any:
        ...
        # PaperQA配置
        elif name == "PAPER_QA_TIMEOUT":
            return int(cls._get_env("PAPER_QA_TIMEOUT", "900"))  # 15分钟总超时
        elif name == "PDF_PARSE_TIMEOUT":
            return int(cls._get_env("PDF_PARSE_TIMEOUT", "180"))  # 3分钟
        elif name == "QUESTION_ANALYSIS_TIMEOUT":
            return int(cls._get_env("QUESTION_ANALYSIS_TIMEOUT", "60"))  # 1分钟
        elif name == "PASSAGE_RETRIEVAL_TIMEOUT":
            return int(cls._get_env("PASSAGE_RETRIEVAL_TIMEOUT", "120"))  # 2分钟
        elif name == "EVIDENCE_FILTERING_TIMEOUT":
            return int(cls._get_env("EVIDENCE_FILTERING_TIMEOUT", "120"))  # 2分钟
        elif name == "ANSWER_GENERATION_TIMEOUT":
            return int(cls._get_env("ANSWER_GENERATION_TIMEOUT", "300"))  # 5分钟
        
        # 文本分块配置
        elif name == "CHUNK_SIZE":
            return int(cls._get_env("CHUNK_SIZE", "1000"))
        elif name == "CHUNK_OVERLAP":
            return int(cls._get_env("CHUNK_OVERLAP", "200"))
        elif name == "TOP_K_PASSAGES":
            return int(cls._get_env("TOP_K_PASSAGES", "8"))
    ...
    def print_config(cls):
        ...
        print(f"PaperQA总超时: {cls.PAPER_QA_TIMEOUT}秒")
        print("================")
